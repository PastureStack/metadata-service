...
func GenerateAnswers(data []map[string]interface{}) (Versions, error) {
	versions, _, err := GenerateScopedAnswers(data, true)
	return versions, err
}

func GenerateScopedAnswers(data []map[string]interface{}, local bool) (Versions, []Credential, error) {
	versions := make(map[string]Answers)
	versionsToGenerate := versionList
	if !local {
		versionsToGenerate = []string{version3}
	}
	var credentials []Credential

	for _, v := range versionsToGenerate {
		...
		credentials = append([]Credential(nil), interim.Credentials...)
		// 2. Generate versions from temp data
		if err := generateVersions(interim, v, versions, local); err != nil {
			return nil, nil, err
		}
	}
	...
	return versions, credentials, nil
}

func generateVersions(interim *Interim, version string, versions Versions, local bool) error {
	versionedData, err := applyVersionToData(*interim, version)
	if err != nil {
		return err
	}
	addToVersions(versions, version, versionedData, local)
	return nil
}

func addToVersions(versions Versions, version string, versionedData *Interim, local bool) {
...
func addEnvironment(environment map[string]interface{}, interim *Interim) {
	interim.Environment = environment
}

func addCredential(data map[string]interface{}, interim *Interim) {
	credential := Credential{}
	credential.URL, _ = data["url"].(string)
	credential.PublicValue, _ = data["public_value"].(string)
	credential.SecretValue, _ = data["secret_value"].(string)
	if credential.URL == "" || credential.PublicValue == "" || credential.SecretValue == "" {
		return
	}
	interim.Credentials = append(interim.Credentials, credential)
}

...
func newStaticTestServerConfig(t *testing.T, versions Versions) *ServerConfig {
	t.Helper()
	config := NewServerConfig(filepath.Join(t.TempDir(), "answers.json"), "127.0.0.1:0", "127.0.0.1:0", false, false, 1)
	config.metadataController.SetStaticVersions(versions)
	return config
}

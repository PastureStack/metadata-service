...
func TestKickCoalescesPendingWorkWithoutLosingIt(t *testing.T) {
	...
	if firstGeneration != 0 {
		t.Fatalf("first generation = %d, want 0", firstGeneration)
	}
	...
	if pendingGeneration != firstGeneration {
		t.Fatalf("pending generation = %d, want %d", pendingGeneration, firstGeneration)
	}
	...
	release <- struct{}{}
	waitWithin(t, k, firstGeneration+1)
